...
public class WorldSerializer extends ReadOnlySerializer<World>
{
	...
	private final DecorSerializer 	decorSerializer;
	...
	public WorldSerializer(RubeScene scene, Json _json)
	{
		...
		decorSerializer = new DecorSerializer(scene);
		_json.setSerializer(RubeDecor.class, decorSerializer);
	}
	...
	public World read(Json json, JsonValue jsonData, Class type) 
	{
	   ...
	   if (world == null)
	   {
		...
		if (images != null) {
		   if (scene.getImages() == null) scene.setImages(images);
		   else scene.getImages().addAll(images);
		...
		// Decorations
		Array<RubeDecor> decors = json.readValue("decorations", Array.class, RubeDecor.class, jsonData);
		if (decors != null) {
		   if (scene.getDecors() == null) scene.setDecors(decors);
		   else scene.getDecors().addAll(decors);
		}
