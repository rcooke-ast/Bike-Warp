...
public class RubeScene 
{
	...
	private Array<RubeDecor> mDecors;
	...
	public void parseCustomProperties(Json json,Object item, JsonValue jsonData)
	{
		...
		if (customProperties != null)
		{
			for (Map<String, ?> property : customProperties)
			{
				...
				else if (property.containsKey("vertices"))
				{
					RubeVertexArray vertices = json.readValue(RubeVertexArray.class, (JsonValue)property.get("vertices"));
					//PolygonShape shape = new PolygonShape();
					//shape.set(vertices.toVector2());
					setCustom(item, propertyName, vertices); 
				}
			}
		}
	}
   ...
   public void clear() {
	   if (mBodies != null) mBodies.clear();
	   if (mFixtures != null) mFixtures.clear();
	   if (mJoints != null) mJoints.clear();
	   if (mImages != null) mImages.clear();
	   if (mDecors != null) mDecors.clear();
	   if (mCustomPropertiesMap != null) mCustomPropertiesMap.clear();
	   if (mBodyImageMap != null) mBodyImageMap.clear();
   ...
   public void setDecors(Array<RubeDecor> decors)
   {
      this.mDecors = decors;
   }
   
   public Array<RubeDecor> getDecors()
   {
      return mDecors;
   }
