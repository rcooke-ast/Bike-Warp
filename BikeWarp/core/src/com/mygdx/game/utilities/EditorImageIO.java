...
public class EditorImageIO {
	...
	public static int ImageMoveableSign(JSONStringer json, float[] fpo, int objType, int bodyIndex, int cnt) throws JSONException {
		String imageFile = DecorVars.GetRoadSignFilename(objType-100);
		float rd = 30.0f*B2DVars.EPPM;
		float shaftLength = 5.0f*rd;
		float[] xshaft = {-0.03f, 0.03f, 0.03f, -0.03f};
		float[] yshaft = {-shaftLength*0.5f, -shaftLength*0.5f, shaftLength*0.5f, shaftLength*0.5f};
		PolygonOperations.RotateArray(xshaft,yshaft,MathUtils.radiansToDegrees*fpo[3],0.0f,shaftLength*0.5f);
		float[] cenv = PolygonOperations.RotateCoordinate(0.0f, -0.5f*shaftLength, MathUtils.radiansToDegrees*fpo[3], 0.0f, 0.0f);
		json.object(); // Start of Road Sign Post
		...
		json.key("scale").value(1);
		json.key("aspectScale").value(1);
		json.key("angle").value(fpo[3]);
		json.key("body").value(bodyIndex);
		json.key("center");
		json.object();
		json.key("x").value(cenv[0]);
		json.key("y").value(cenv[1]);
//		json.key("x").value((fpa[2]-fpo[0])*B2DVars.EPPM);
//		json.key("y").value((fpa[3]-fpo[1])*B2DVars.EPPM +0.5f*shaftLength);
		json.endObject();
		json.key("corners");
...
//		json.key("glVertexPointer").array().value(-0.03f).value(-0.5f*shaftLength).value(0.03f).value(-0.5f*shaftLength).value(0.03f).value(0.5f*shaftLength).value(0.03f).value(0.5f*shaftLength).endArray();
		json.endObject(); // End of Road Sign Post
		json.object();
		json.key("name").value("Moveable"+cnt);
		json.key("opacity").value(1);
		json.key("renderOrder").value(0);
		json.key("scale").value(1);
		json.key("aspectScale").value(1);
