...
public class BayazitDecomposer {
	...
	private static Vector2 at(int i, ArrayList<Vector2> vertices) {
		int s = vertices.size();
		return vertices.get(i < 0 ? s - (-i % s) : i % s);
	}

	private static ArrayList<Vector2> copy(int i, int j, ArrayList<Vector2> vertices) {
	...
	public static float getSignedArea(ArrayList<Vector2> vect) {
		int i;
		float area = 0;
	...
	public static Boolean isCounterClockWise(ArrayList<Vector2> vect) {
	...
	// / <returns></returns>
	public static ArrayList<ArrayList<Vector2>> convexPartition(ArrayList<Vector2> vertices) {
	...
	private static Boolean canSee(int i, int j, ArrayList<Vector2> vertices) {
		if (reflex(i, vertices)) {
	...
	// otherwise.</returns>
	public static Boolean lineIntersect(Vector2 point1, Vector2 point2, Vector2 point3,
			Vector2 point4, Boolean firstIsSegment, Boolean secondIsSegment, Vector2 point) {
		point = new Vector2();
	...
	// precondition: ccw
	private static Boolean reflex(int i, ArrayList<Vector2> vertices) {
		return right(i, vertices);
	}

	private static Boolean right(int i, ArrayList<Vector2> vertices) {
...
class SimplifyTools {
	...
	public static ArrayList<Vector2> collinearSimplify(ArrayList<Vector2> vertices,
	...
	public static ArrayList<Vector2> douglasPeuckerSimplify(ArrayList<Vector2> vertices,
	...
	private static void simplifySection(ArrayList<Vector2> vertices, int i, int j) {
		if ((i + 1) == j) return;
	...
	// From physics2d.net
	public static ArrayList<Vector2> reduceByArea(ArrayList<Vector2> vertices, float areaTolerance) {
		if (vertices.size() <= 3) return vertices;
		if (areaTolerance < 0) {
	...
	public static void mergeParallelEdges(ArrayList<Vector2> vertices, float tolerance) {
	...
	// / <param name="vertices">The vertices.</param>
	// / <returns></returns>
	public static ArrayList<Vector2> mergeIdenticalPoints(ArrayList<Vector2> vertices) {
	...
	// / <returns></returns>
	public static ArrayList<Vector2> reduceByNth(ArrayList<Vector2> vertices, int nth) {
		// We can't simplify polygons under 3 vertices
		if (vertices.size() < 3) return vertices;
