...
public class OptionChangeHUDDisplay extends GameState {
    private static final String[] options = {"Options Menu", "Timer", "Personal Record", "World Record", "Emerald Count", "Key Count", "Nitrous Count"};
	...
	public OptionChangeHUDDisplay(GameStateManager gsm) {
        super(gsm);
        create();
    }
    ...
    public void handleInput() {
        if (GameInput.isPressed(GameInput.KEY_UP)) {
            ...
            if (currentOption==0) {
                ...
                // Toggle the options
                int currKey = GameVars.GetPlayerDisplay(currentOption-1);
                currKey++;
                if (currentOption<=3) {
                    if (currKey > 1) currKey = 0;
                } else {
                    if (currKey > 2) currKey = 0;
                }
                GameVars.SetPlayerDisplay(currentOption-1,currKey);
            }
    ...
    public void render() {
        ...
        if (keyNotAllowed == 1) {
            ...
            else displayText = "Press enter to toggle display of the " + options[currentOption].toLowerCase() + ".\n\nThe current setting is:\n" + HUDDisplyOption(GameVars.plyrDisplay.get(GameVars.currentPlayer)[currentOption-1]);
        }
    ...
    private String HUDDisplyOption(int value) {
	    if (value == GameVars.DISPLAY_OFF) return "Off";
        else if (value == GameVars.DISPLAY_ON) return "On";
        else if (value == GameVars.DISPLAY_SMARTON) return "Smart On";
        return "Off";
    }
