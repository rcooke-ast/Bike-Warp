 ...
 * @author rcooke
 */
public class LevelSelectTraining extends GameState {
    ...
    public void handleInput() {
    	if (GameInput.isPressed(GameInput.KEY_UP)) {
        	...
        	if (currentOption==0) fadeOut=1.0f; // Return to Main Menu
        		...
        		// Load the level
        		gsm.setState(GameStateManager.PLAY, true, EditorIO.loadLevelPlay(Gdx.files.internal(LevelsListTraining.trainingLevelFiles[currentOption])), currentOption, true);
        	}
        } else if (fadeOut==0.0f) {
    		fadeOut=-1.0f;
    		gsm.setState(GameStateManager.PEEK, false, "none", currentOption, true);
        }
    ...
    public void render() {
        ...
        for (int i=numMin; i<numMin+numLevShow; i++) {
        	...
        	lvlWidth = menuText.getBounds(LevelsListTraining.trainingLevelNames[i]).width;
        	menuText.draw(sb, LevelsListTraining.trainingLevelNames[i], cam.position.x-0.25f*(SCRWIDTH-0.075f*BikeGame.V_HEIGHT)-lvlWidth/2, cam.position.y + (1.5f*menuHeight*numLevShow)/2 - 1.5f*(i-numMin)*menuHeight);
        }
